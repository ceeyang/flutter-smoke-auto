...
闸门与编排工具都经过 TDD（58 个回归用例），且针对实践中真实观察到的绕过手法做过加固——例如源码对账要求 identifier *带引号*出现，因为"在注释里提一句 id"曾是骗过它的最省力方式。
...
**7. 定向执行让它日常可持续——全量是显式选择。** `run_smoke.sh` **拒绝裸跑**：必须声明范围（`--changed` / `--only <功能>` / `--failed` / `--from-list` / `--all`）。`--changed` 把 `git diff` → 契约表 `file` 字段 → 受影响用例（subflow 引用反查主 flow，逻辑层改动按 feature 目录回退映射）自动圈出来，永远附带冷启动锚点——几分钟而不是一整轮；`--failed` 只重跑上一轮的失败用例。关键词没命中会硬拒绝并列出可选用例名——那条曾诱导 agent"干脆全量"的静默退化路已经堵死。四类改动仍强制全量（影响面是全局的）：路由/导航、全局状态、主题/国际化、依赖或 Flutter 版本升级。失败率超 50% 触发熔断提示：那是一个坏掉的公共前置，不是二十个独立 bug。

**8. 该并行的并行，该串行的串行。** 并行是**声明**出来的不是假设出来的：用例用 `tags` 声明资源占用（`mutates-posts` / `readonly`），`shard_flows.py` 把写冲突（含传递闭包）的用例放进同一条串行车道，其余摊平；未声明的保守串行。`device_pool.py` 给每个会话独占自己的模拟器（跨会话注册表、用户 pin 分配、每端上限 + 实时内存预算）——在 iOS 上这是保命的：同一台模拟器两个自动化会话会打崩 SpringBoard。Web 并行是原生的：Playwright worker 各自独立 browser context（cookie/缓存互不可见），`laneEnv()` 按 worker 分发测试账号。多端验收默认并行执行（每端一个只读子代理），修复集中做、按根因去重。

**9. 工程上经得起看。** 闸门脚本零第三方依赖（内置 Maestro YAML 子集迷你解析器，不引 PyYAML），兼容 macOS 系统 bash 3.2；58 个回归用例全部先写红（先复现作弊路径再修）；针对实践中的真实绕过手法加固过。视觉兜底（截图 + 模型定位 + 坐标点击）是最后手段：每次点击都有像素差分验证，整条用例都活在这一层本身就会被报告为可访问性缺陷。

## 仓库内容

| 路径 | 说明 |
|---|---|
| `flutter-smoke-auto/` | 主 skill：SKILL.md（工作流）、7 份参考文档、8 个脚本、CI/hook/flow 模板、闸门回归测试 |
| `smoke-all/` | `/smoke-all` —— 所有可用端全量冒烟验收，三端并行（大套件可叠加端内车道） |
...
- `select_flows.py` —— git diff / 关键词 / 上轮失败 → 受影响用例映射，支撑定向执行
- `shard_flows.py` —— 按资源写冲突分车道，支撑端内并行
- `device_pool.py` —— 跨会话模拟器所有权注册表（claim / pin / 上限 / 内存预算）
